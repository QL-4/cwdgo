...
Grab `cwdgo.exe` from the [latest release](https://github.com/QL-4/cwdgo/releases/latest), drop it anywhere, double-click. That's it — it lives in your tray.

Each release ships a `cwdgo.exe.sha256` next to the binary if you want to verify the download:

```powershell
(Get-FileHash cwdgo.exe -Algorithm SHA256).Hash.ToLower()
```

> Windows SmartScreen may warn on first run since the binary is unsigned — *More info → Run anyway*.
...
# Install the Wails CLI (one-time; same version CI builds with)
